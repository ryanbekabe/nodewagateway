...
// 🔹 Endpoint baru untuk mengirim pesan dengan lampiran
app.post("/send-message-lampiran", [
    body("token").notEmpty().withMessage("Token is required"),
    body("number").notEmpty().withMessage("Number is required"),
    ...
    const { token, number, mediaUrl, message } = req.body;
    const userId = tokens[token];
    if (!userId || !users[userId] || users[userId].token !== token) {
        return res.status(403).json({ status: false, message: "Invalid token or user not registered" });
    }
    ...
    try {
        const recipient = number + "@s.whatsapp.net";
        const mediaMessage = { image: { url: mediaUrl }, caption: message || "" };
        await users[userId].sock.sendMessage(recipient, mediaMessage);
        res.json({ status: true, message: "Media message sent successfully" });
    } catch (error) {
        res.status(500).json({ status: false, message: "Failed to send media message", error });
    }
});
